...
Claude Desktop AI가 스스로:
  📋 plan_task("AI 윤리 발표 12장") → 마일스톤 수립, 목표 저장
  🔍 search_papers("AI 윤리") → 논문 15편 수신, 관련 6편 선별
  📋 register_references([...6편...]) → ref_001~ref_006 등록
  ✍️  Marp 마크다운 12장 직접 작성
  💾 save_output(ppt) → ~/Desktop/AI_윤리_기말발표_2026-03-08.pdf
  📚 참고문헌 6편 자동 삽입 (APA 형식)
```

도중에 오류가 나도 **자동으로 복구**합니다. 컨텍스트를 잃어도 **목표를 기억**합니다.
...
✅ uni-agent (에이전트 방식)
사용자 → "발표 준비해줘"
에이전트 → 목표 등록 → 논문 조사 → 초안 작성 → PDF 출력
(전체 과정을 에이전트가 스스로 처리, 오류 시 자동 복구)
```

### MCP 도구 (Claude Desktop 노출)

6개 도구를 노출합니다. Claude Desktop AI가 직접 순서를 판단하며 호출합니다.

| 도구 | 역할 | 페르소나 |
|------|------|---------|
| `plan_task` | 목표 설정, 마일스톤 수립, 상태 저장 | Architect |
...
| `parse_file` | 첨부파일(PDF/PPTX/DOCX/HWP) 텍스트 추출 | Reader |
...
| 명령 | 동작 |
|------|------|
| `/ppt <topic>` | plan_task → 논문 검색 → 등록 → Marp 12슬라이드 → PDF 저장 |
| `/report <topic>` | plan_task → 논문 검색 → 등록 → 보고서 → DOCX 저장 |
...
v3.0.0부터 **Fat MCP 패턴**을 채택합니다. MCP 서버가 단순 I/O 중계를 넘어 지능형 중간 레이어로 작동합니다.
...
│                                         │
│  Pre-Hook: 보안 검사 · 컨텍스트 주입    │
│      ↓                                  │
│  Role Router: 도구별 페르소나 결정      │
│      ↓                                  │
│  I/O 도구 실행 (6개)                    │
│      ↓                                  │
│  Post-Hook: 에러 복구 · 상태 갱신       │
│      ↓                                  │
│  State Manager: ~/.uni-agent/state.json │
└─────────────────────────────────────────┘
    │
    ▼
외부 API (인증 불필요)    파일 출력
...
도구 실행 중 오류가 발생하면 서버가 자동으로 복구 신호를 주입합니다. 최대 3회까지 재시도하며, 초과 시에만 사용자에게 오류를 노출합니다.

```
도구 실행 실패
  └─ Post-Hook: 에러 숨김 + 복구 신호 주입
...
```
~/.uni-agent/
├── state.json             ← 현재 목표·마일스톤·retry 이력 (Fat MCP 상태)
├── references/
│   ├── index.json         ← 레퍼런스 메타데이터
│   └── originals/         ← 원본 파일
├── vector-store/
│   ├── index.bin          ← HNSW 벡터 인덱스 (768차원)
│   └── metadata.json      ← 청크 메타데이터
├── sessions/
│   └── {sessionId}.json   ← 세션 상태
├── plugins/               ← 커뮤니티 플러그인
├── tmp/                   ← 변환 임시 파일
└── embedding-cache.json   ← 임베딩 캐시 (영구)
```
...
| LLM | Claude Desktop AI 자신 (Fat MCP) | 판단·작성, MCP 서버에서 LLM 호출 없음 |
...
6. **추가 비용 없음** — Claude 구독 외 별도 결제 불필요
...
| [`agent_docs/agents.md`](./agent_docs/agents.md) | MCP 도구 6개 명세, Hook 레이어, Role Router, State Manager |
...
| [`agent_docs/workflow.md`](./agent_docs/workflow.md) | Fat MCP 워크플로우, Hook 흐름, 자율 복구 루프 |
